...
def oversample_demographic_data(X_train, y_train, train_demographics, group_condition=None):
    """
    Oversample a specific demographic group or the entire dataset.
    ...
    y_train : Series
        Training labels
    train_demographics : dict
        Dictionary containing demographic information
    group_condition : Series, optional
        Boolean mask indicating which samples belong to specific demographic group
    ...
    if group_condition is not None:
        # Extract demographic info aligned with X_train
        group_mask_train = group_condition.loc[X_train.index]
        X_group = X_train[group_mask_train]
        y_group = y_train[group_mask_train]
    else:
        X_group = X_train
        y_group = y_train
    
    # Check if we have enough samples for oversampling
    if len(X_group) < 10 or len(np.unique(y_group)) < 2:
        return X_group.values, y_group.values
    
    # Apply SMOTE for oversampling
    try:
        smote = SMOTE(random_state=42)
        X_resampled, y_resampled = smote.fit_resample(X_group, y_group)
        print(f"Oversampled data: {len(X_group)} -> {len(X_resampled)} samples")
        return X_resampled, y_resampled
    except ValueError as e:
        # Fallback to RandomOverSampler if SMOTE fails
        print(f"SMOTE failed: {e}. Using RandomOverSampler instead.")
        ros = RandomOverSampler(random_state=42)
        X_resampled, y_resampled = ros.fit_resample(X_group, y_group)
        print(f"Oversampled data: {len(X_group)} -> {len(X_resampled)} samples")
        return X_resampled, y_resampled

def create_demographic_models(df, X, y, intersectional_groups):
    """Train optimized models for each demographic group with oversampling."""
    demographic_models = {}
    ...
    for group_name, group_condition in intersectional_groups:
        ...
        # Apply oversampling to training data for this group
        try:
            smote = SMOTE(random_state=42)
            X_train_group_resampled, y_train_group_resampled = smote.fit_resample(X_train_group, y_train_group)
            print(f"Applied SMOTE oversampling: {len(X_train_group)} -> {len(X_train_group_resampled)} samples")
        except ValueError as e:
            # Fallback to RandomOverSampler if SMOTE fails
            print(f"SMOTE failed: {e}. Using RandomOverSampler instead.")
            ros = RandomOverSampler(random_state=42)
            X_train_group_resampled, y_train_group_resampled = ros.fit_resample(X_train_group, y_train_group)
            print(f"Applied RandomOverSampler: {len(X_train_group)} -> {len(X_train_group_resampled)} samples")
        
        # Standardize features
        scaler_group = StandardScaler()
        X_train_group_scaled = scaler_group.fit_transform(X_train_group_resampled)
        ...
        model_group.fit(X_train_group_scaled, y_train_group_resampled)
