# mitigation_approach_final.py - Ensemble with Threshold Calibration
...
def prepare_data_for_modeling(df):
    ...
    intersectional_groups = [
        ...
        ("Age_30s-40s", df['Age Group'] == '30s-40s'),
        ("Age_50s", df['Age Group'] == '50s'),
        ("Age_60+", df['Age Group'] == '60+'),
        ("Male_50s", (male_condition) & (df['Age Group'] == '50s')),
        ("Female_50s", (female_condition) & (df['Age Group'] == '50s')),
        ("Male_30s-40s", (male_condition) & (df['Age Group'] == '30s-40s')),
        ("Female_30s-40s", (female_condition) & (df['Age Group'] == '30s-40s')),
        ("Male_60+", (male_condition) & (df['Age Group'] == '60+')),
        ("Female_60+", (female_condition) & (df['Age Group'] == '60+'))         
    ]
...
def create_demographic_models(df, X, y, intersectional_groups):
    """Train optimized models for each demographic group with 5-fold CV."""
    demographic_models = {}
    cv_results = {}
    ...
    for group_name, group_condition in intersectional_groups:
        ...
        for train_idx, val_idx in skf.split(X_train_group_scaled, y_train_group):
            # Get train and validation sets for this fold
            X_fold_train, X_fold_val = X_train_group_scaled[train_idx], X_train_group_scaled[val_idx]
            y_fold_train, y_fold_val = y_train_group.iloc[train_idx], y_train_group.iloc[val_idx]
            
            # Train model on this fold
            fold_model = LogisticRegression(C=1.0, solver='liblinear', random_state=42, max_iter=1000)
        ...
        model_group = LogisticRegression(C=1.0, solver='liblinear', random_state=42, max_iter=1000)
        ...
        print(f"CV Performance for {group_name} model:")
        print(f"CV Accuracy: {cv_metrics['accuracy']:.4f}")
        print(f"CV F1 Score: {cv_metrics['f1']:.4f}")
        print(f"Test Performance for {group_name} model:")
        print(f"Test Accuracy: {test_metrics['accuracy']:.4f}")
        print(f"Test F1 Score: {test_metrics['f1']:.4f}")
        
        # Store the model, metrics and related data
        ...
        cv_results[group_name] = cv_metrics
    
    return demographic_models, X_train, X_test, y_train, y_test, train_demographics, test_demographics, cv_results
...
def calculate_gap_with_threshold(threshold, group_probs, group_y_test, all_groups_f1):
    """Calculate how a threshold for one group affects the overall performance gap"""
    preds = (group_probs >= threshold).astype(int)
    group_f1 = f1_score(group_y_test, preds, zero_division=0)
    
    # Calculate gap between this group's F1 and other groups
    gaps = [abs(group_f1 - other_f1) for other_f1 in all_groups_f1 if other_f1 > 0]
    if not gaps:
        return 0
    return sum(gaps) / len(gaps)  # Average gap

def calibrate_thresholds(prediction_df, X_test, y_test, test_demographics, demographic_models, intersectional_groups):
    """
    Calibrate prediction thresholds for each demographic group to optimize performance and fairness.
    """
    ...
    # Calculate baseline F1 scores for all groups to use in gap calculation
    all_baseline_f1 = [metrics['f1'] for metrics in baseline_metrics.values() if metrics['f1'] > 0]
    ...
    for group_name, mask in group_masks.items():
        ...
        model_weights = {
            ...
            'Age_30s-40s': 1.5,
            'Age_50s': 1.0,
            'Age_60+': 2.5,  # Increased from 2.0
            'Male_50s': 1.5,
            'Female_50s': 1.0,
            'Male_30s-40s': 1.0,
            'Female_30s-40s': 1.0,
            'Male_60+': 3.0,  # Significant increase as this group declined most
            'Female_60+': 1.5
        }
        ...
        for model_name, model_info in demographic_models.items():
            ...
            # Weight by model performance or specific model weight
            group_weight = model_weights.get(model_name, 1.0)
        ...
        total_weight = sum(model_weights.get(model_name, 1.0) * (1.5 if model_name == group_name else 1.0) 
        ...
        # Define fairness objective function for this group
        def fairness_objective(threshold):
            preds = (group_probs >= threshold).astype(int)
            group_f1 = f1_score(group_y_test, preds, zero_division=0)
            
            # Calculate how this threshold affects the performance gap
            gap = calculate_gap_with_threshold(threshold, group_probs, group_y_test, all_baseline_f1)
            
            # Combined objective: maximize F1 while minimizing gap
            # The 0.5 is a weighting factor - adjust as needed
            fairness_weight = 0.5
            return -(group_f1 - fairness_weight * gap)  # Negative because we're minimizing
        ...
        for threshold in thresholds_to_try:
            objective_value = fairness_objective(threshold)
        ...
        if group_name in baseline_metrics:
            ...
            preds = (group_probs >= best_threshold).astype(int)
            best_f1 = f1_score(group_y_test, preds, zero_division=0)
            
            # If even the best threshold doesn't improve over baseline by a meaningful amount,
            # stick closer to the default threshold
            if best_f1 < baseline_f1 * 1.05:  # 5% improvement threshold
                # Try to get as close to baseline performance as possible
                min_diff = float('inf')
                best_backup_threshold = 0.5
                
                for threshold in thresholds_to_try:
                    preds = (group_probs >= threshold).astype(int)
                    curr_f1 = f1_score(group_y_test, preds, zero_division=0)
                    diff = abs(curr_f1 - baseline_f1)
                    
                    if diff < min_diff:
                        min_diff = diff
                        best_backup_threshold = threshold
                
                best_threshold = best_backup_threshold
...
def apply_calibrated_predictions(X_test, y_test, demographic_models, test_demographics, intersectional_groups, thresholds):
    ...
    model_weights = {
        ...
        'Age_30s-40s': 1.5,
        'Age_50s': 1.0,
        'Age_60+': 2.5,  # Increase from 2.0
        'Male_50s': 1.5,
        'Female_50s': 1.0,
        'Male_30s-40s': 1.0,  # Decrease slightly as this group is doing very well
        'Female_30s-40s': 1.0,
        'Male_60+': 3.0,  # Significant increase as this group declined most
        'Female_60+': 1.5
    }
    ...
    priority_order = [
        # Most specific intersectional groups first
        'Male_30s-40s', 'Female_30s-40s', 'Male_50s', 'Female_50s', 'Male_60+', 'Female_60+',
        # Age groups next
        'Age_30s-40s', 'Age_50s', 'Age_60+',
        # Gender groups next
        'Male', 'Female',
        # Global model last
        'All'
...
def visualize_ensemble_results(results, fairness_metrics):
    ...
    # Set up data for fairness metrics
    fairness_data = {
        'Metric': ['Average Performance Gap', 'Maximum Performance Gap'],
        'Baseline': [fairness_metrics['avg_abs_gap']['baseline'], 
                    fairness_metrics['max_abs_gap']['baseline']],
        'Ensemble': [fairness_metrics['avg_abs_gap']['ensemble'], 
                   fairness_metrics['max_abs_gap']['ensemble']]
    }
    
    fairness_df = pd.DataFrame(fairness_data)
    
    # Convert to long format
    fairness_long = pd.melt(
        fairness_df,
        id_vars=['Metric'],
        value_vars=['Baseline', 'Ensemble'],
        var_name='Model',
    ...
    sns.barplot(x='Metric', y='Gap', hue='Model', data=fairness_long)
    plt.title('Fairness Metrics Comparison')
    plt.ylabel('Performance Gap')
    plt.ylim(0, max(fairness_metrics['max_abs_gap'].values()) * 1.2)
...
def main():
    ...
    try:
        ...
        demographic_models, X_train, X_test, y_train, y_test, train_demographics, test_demographics, cv_results = (
        ...
        # Visualize results
        visualize_ensemble_results(results, fairness_metrics)
